...
static	char	*ft_mprintf_lst_flush(t_list *lst);
...
char	*ft_mprintf(const char *str, ...)
{
	...
	if (ft_printf_lstcheck(format_str))
	{
		...
		{
			r = ft_mprintf_lst_flush(printable_str);
		}
...
static char	*ft_mprintf_lst_flush(t_list *lst)
{
	...
	r = ft_calloc(sizeof(char), count + 3);
	count = 0;
	while (lst)
	{
		len = ft_strlen(((char *) lst->content) + 1);
		ft_strlcpy(&r[count], (char *) lst->content + 1, len + 1);
		count += len;
		lst = lst->next;
	}
	return (r);
