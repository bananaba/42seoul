...
/*   Updated: 2022/10/25 21:16:45 by balee            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	add_env(t_list *envp, char **str)
{
	...
	lst = ft_lstlast(envp);
	lst->next = ft_lstnew(ft_strjoin(temp, str[1]));
	free(temp);
}
...
t_list	*sort_env(t_list *envp)
{
	...
	while (envp)
	{
		...
		while (temp1->next && ft_strncmp(temp1->next->content, envp->content,
			...
			temp1 = temp1->next;
		if (temp1->next == NULL)
			temp1->next = ft_lstnew(ft_strdup(envp->content));
		else
		{
