...
/*   Updated: 2022/10/26 03:52:19 by balee            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	variable_expansion(t_list *token_start, t_list *envp)
{
	...
	while (now)// 토큰을 순회하면서
	{
		...
		if (now_token->type == "text"[0] || now_token->type == "double_quote"[0])//만약 토큰이 text이거나 double_quote이면
		{
			...
			{
				ft_lstclear(&token_start, (void (*)(void *))destroy_token);// 토큰 연결리스트를 싹 지우고
				return (1);//1을 리턴한다.
			}
		}
		if (DEBUG)
...
char	*get_envp(char *str, t_list *envp)
{
	...
	while (now)//envp를 순회한다.
	{
		if (ft_strncmp(str, now->content,
				int_max(ft_strlen_c(now->content, '='), ft_varlen(str))) == 0)// str과 envp중 하나를 비교한다. -> 비교 길이는 (str의 변수길이와, envp의 변수 길이) 중 긴걸로 한다.
		{
